...
IF OBJECT_ID('dbo.SP_SettleClaimWithPayments', 'P') IS NOT NULL DROP PROCEDURE dbo.SP_SettleClaimWithPayments;
IF OBJECT_ID('dbo.SP_BulkVerifyClaims', 'P') IS NOT NULL DROP PROCEDURE dbo.SP_BulkVerifyClaims;
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @AssetCount > 0
        BEGIN
            SET @ErrorMessage = 'Cannot delete deceased record with ' + CAST(@AssetCount AS NVARCHAR(10)) + ' associated assets. Use SP_DeleteDeceasedWithAssets for cascade deletion.';
            ROLLBACK TRANSACTION;
            RETURN;
        END
...
-- Stored Procedure: Create Asset with Validation (Updated to handle detail tables)
...
ALTER PROCEDURE dbo.SP_CreateUserByAdmin
    @Username NVARCHAR(100),
    @PasswordHash VARBINARY(256),
    @Role NVARCHAR(50),
    @CreatedByUserId INT,
    @Email NVARCHAR(200) = NULL,
    @CreateSQLLogin BIT = 1, -- New parameter: whether to create SQL Server login
    @SQLLoginPassword NVARCHAR(255) = NULL, -- Password for SQL Server login (if different from app password)
    @NewUserId INT OUTPUT,
    @ErrorMessage NVARCHAR(500) OUTPUT
AS
